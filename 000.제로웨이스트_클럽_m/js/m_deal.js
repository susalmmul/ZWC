...
function make_total_price() {
    let item_cur_qty = 0;
console.log("4444: " + $('.qty_num').length);    
    for(let i=0; i<$('.qty_num').length; i++) { // 클래스가 qty_num인 것의 모든 밸류값을 더해서 총 액과 곱해야 하니까.

        let item_qty = Number($('.qty_num').eq(i).val());
        let item_price = Number($('.item_price').eq(i).children('span').text().replace(',',"").replace('원',""))

        item_cur_qty += item_qty * item_price;


console.log( item_price)
    }

console.log("여기: " + item_cur_qty)

    $('.total_price > span').text(comma(item_cur_qty) + "원")
}

// 구매 버튼 누르면 옵션창 뜨는 기능

$('.cart_btn').click(()=>{
    $('.opt_fixed').addClass('opt_fixed_up');
    $('.opt_btn').css({ display: "block"});
});

// 옵션 버튼 기능

let opt_btn_chk = true;
$('.opt_btn').click(function(){
    if (opt_btn_chk) {
        $(this).removeClass('opt_btn_close')
        $('.opt_fixed').removeClass('opt_fixed_up');
        opt_btn_chk = false;
    }
    else {
        $(this).addClass('opt_btn_close')
        $('.opt_fixed').addClass('opt_fixed_up');
        opt_btn_chk = true;
    }
});
...
$('.l_content_area').append(`<img src="../000.제로웨이스트_클럽/img/shop_page/${MENU_TITLE[cate_no]}/${data.src}" alt="">`)
